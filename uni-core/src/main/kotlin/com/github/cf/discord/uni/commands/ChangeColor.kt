...
class ChangeColor : Command(){
    ...
    override fun run(ctx: Context) {
        ...
        if(sp == null){
            ctx.guild.removeRoleFromMember(user!!, serverRoles.find { it.name.startsWith("#", true) }!!).queue()
            ...
            ctx.guild.removeRoleFromMember(user!!, serverRoles.find { it.name.startsWith("#", true) }!!).queue()
            val newRole = ctx.guild.createRole().setColor(color).setName("#$colorName").complete()
            ctx.guild.addRoleToMember(user, newRole).queue()
            ...
            if(roleId == null){
                ...
                ctx.guild.removeRoleFromMember(user!!, serverRoles.find { it.name.startsWith("#", true) }!!).queue()
                val haha = ctx.guild.createRole().setColor(color).setName("#$hexAss").complete()
                ctx.guild.addRoleToMember(user, haha).queue()
                ...
                ctx.guild.addRoleToMember(ctx.guild.getMember(ctx.author)!!, role!!).queue()
                ctx.send("<@!${ctx.author.idLong}> changed your color!")
            }
        }
    }
